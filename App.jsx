...
const PRESETS = {
  2: [
    ...
    [1,11,12,14,13,15,16,17,18,19,20],
  ],
  3: [
    [1,2,3,4,5,6,7],
    [1,8,9,10,11,12,14],
...
function DriverView({ numCars, routeIdx }) {
  ...
  const [showDetails, setShowDetails] = useState(false); // false=waze, true=details
  ...
  function goBack() {
    ...
    if (prevId) { setDelivered(d=>({...d,[prevId]:false})); setShowDetails(false); }
  }
  ...
  function openWaze() {
    const navName = addr.navName || addr.name;
    window.open(`https://waze.com/ul?q=${encodeURIComponent(navName+", "+CITY)}&navigate=yes`, "_blank");
  }
  ...
  // WAZE screen — open waze and show "הגענו" button
  if (!showDetails) return (
    <div style={{ minHeight:"100vh",background:"#111827",direction:"rtl",fontFamily:"'Segoe UI',system-ui,Arial,sans-serif",display:"flex",flexDirection:"column" }}>
      {/* Header */}
      <div style={{ background:route.color,padding:"14px 16px 12px",flexShrink:0 }}>
        <div style={{ display:"flex",alignItems:"center",justifyContent:"space-between" }}>
          <div>
            <div style={{ color:"#fff",fontWeight:800,fontSize:17 }}>{route.emoji} {route.label}</div>
            <div style={{ color:"rgba(255,255,255,0.8)",fontSize:11 }}>עצירה {stopNum} מתוך {total}</div>
          </div>
          <div style={{ background:"rgba(255,255,255,0.22)",borderRadius:20,padding:"4px 14px",fontSize:15,fontWeight:800,color:"#fff" }}>{doneCount}/{total}</div>
        </div>
      ...
      <div style={{ flex:1,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:24,gap:20 }}>
        {/* Destination card */}
        <div style={{ background:"#1f2937",borderRadius:20,padding:"20px 24px",width:"100%",maxWidth:360,textAlign:"center",border:`2px solid ${route.color}44` }}>
          <div style={{ color:"#9ca3af",fontSize:12,marginBottom:6 }}>יעד הבא</div>
          <div style={{ color:"#fff",fontWeight:800,fontSize:22,marginBottom:4 }}>{addr.name}</div>
          {addr.floor && <div style={{ color:"#6b7280",fontSize:14 }}>קומה {addr.floor}{addr.apt?` • דירה ${addr.apt}`:""}</div>}
          {addr.code && <div style={{ color:"#a78bfa",fontSize:14,marginTop:4 }}>🔑 {addr.code}</div>}
        </div>
        ...
        <div style={{ display:"flex",gap:12,width:"100%",maxWidth:360 }}>
          <button onClick={openWaze} style={{
            ...
            background:"#33ccff",color:"#000",fontWeight:800,fontSize:16,cursor:"pointer",
            display:"flex",alignItems:"center",justifyContent:"center",gap:8,
            boxShadow:"0 4px 20px #33ccff44",
          }}>
            <span style={{ fontSize:22 }}>🚗</span> ווייז
          </button>
          <a href={`https://www.google.com/maps/dir/?api=1&destination=${encodeURIComponent((addr.navName||addr.name)+", "+CITY)}`} target="_blank" rel="noreferrer" style={{
            flex:1,padding:"18px 0",borderRadius:16,border:"none",
            background:"#4285f4",color:"#fff",fontWeight:800,fontSize:16,cursor:"pointer",
            display:"flex",alignItems:"center",justifyContent:"center",gap:8,
            textDecoration:"none",boxShadow:"0 4px 20px #4285f444",
          }}>
            <span style={{ fontSize:22 }}>🗺</span> גוגל
          </a>
        </div>

        {/* Arrived button */}
        <button onClick={()=>setShowDetails(true)} style={{
          width:"100%",maxWidth:360,padding:"18px 0",borderRadius:16,border:"2px solid #16a34a",
          background:"transparent",color:"#16a34a",fontWeight:800,fontSize:18,cursor:"pointer",
        }}>
          🏠 הגענו — הצג פרטי בית
        </button>

        {stopNum > 1 && (
          <button onClick={goBack} style={{ background:"none",border:"none",color:"#4b5563",fontSize:13,cursor:"pointer" }}>
            ↩ חזור לעצירה הקודמת
          </button>
        )}
      </div>
    ...
    <div style={{ minHeight:"100vh",background:"#f1f5f9",direction:"rtl",fontFamily:"'Segoe UI',system-ui,Arial,sans-serif",display:"flex",flexDirection:"column" }}>
      <div style={{ background:route.color,color:"#fff",padding:"14px 16px 12px",flexShrink:0 }}>
        <div style={{ display:"flex",alignItems:"center",justifyContent:"space-between" }}>
          <div style={{ display:"flex",alignItems:"center",gap:8 }}>
            <button onClick={()=>setShowDetails(false)} style={{ background:"rgba(255,255,255,0.2)",border:"none",borderRadius:8,color:"#fff",fontSize:14,cursor:"pointer",padding:"4px 10px",fontWeight:600 }}>← חזור</button>
            <div>
      ...
      <div style={{ flex:1,display:"flex",flexDirection:"column",padding:"16px 16px 24px",maxWidth:480,margin:"0 auto",width:"100%",boxSizing:"border-box" }}>
        ...
        <button onClick={markDelivered}
          style={{ width:"100%",padding:"16px 0",borderRadius:14,border:"none",background:"#16a34a",color:"#fff",fontWeight:800,fontSize:18,cursor:"pointer",boxShadow:"0 4px 14px #16a34a55" }}>
          ✓ נמסר — לעצירה הבאה
        </button>
      </div>
    </div>
