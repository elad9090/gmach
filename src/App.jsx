...
function DriverView({ numCars, routeIdx }) {
  ...
  const [showDetails, setShowDetails] = useState(false); // false=map, true=details
  ...
  function markDelivered() {
    ...
    setShowDetails(false); // go back to map for next stop
  }
  function goBack() {
  ...
  // MAP screen — navigate to current stop
  if (!showDetails) return (
    <DriverNav
      ...
      onArrived={() => setShowDetails(true)}
    />
  );

  // DETAILS screen — show when arrived
  return (
      ...
      <div style={{ background:route.color,color:"#fff",padding:"14px 16px 12px",flexShrink:0 }}>
        <div style={{ display:"flex",alignItems:"center",justifyContent:"space-between" }}>
          <div style={{ display:"flex",alignItems:"center",gap:8 }}>
            <button onClick={()=>setShowDetails(false)} style={{ background:"rgba(255,255,255,0.2)",border:"none",borderRadius:8,color:"#fff",fontSize:16,cursor:"pointer",padding:"4px 10px" }}>← מפה</button>
            <div>
