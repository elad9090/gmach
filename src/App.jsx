...
const DEFAULT_ADDRESSES = [
  ...
  { id:11, name:"יפתח הגלעדי 9",      floor:"3",            apt:"12",              details:"" },
...
const PRESETS = {
  1: [
    [1,20,5,4,9,8,3,19,6,7,17,18,14,12,13,15,11,10,2,16],
  ],
  2: [
    [1,20,5,19,8,9,4,10,2,16],
    [1,6,7,17,18,14,13,12,3,15,11],
  ],
  3: [
    [1,8,9,5,20,6,7],
    [1,19,4,10,2,16],
    [1,3,11,15,13,12,14,18,17],
  ],
  4: [
    [1,12,13,14,17,18],
    [1,5,19,3,15,11],
...
function resetIfNewDay() {
  ...
  const savedDay = loadState("gmach_last_day", null);
  if (savedDay !== today) {
    Object.keys(localStorage).forEach(k => {
      if (k.startsWith("gmach_driver_") || k === "gmach_delivered") {
        localStorage.removeItem(k);
      }
    });
    saveState("gmach_last_day", today);
  }
}
resetIfNewDay();
...
function PickCarsScreen() {
  const BG = ["#0891B2","#2563EB","#7C3AED","#059669","#D97706","#DC2626"];
  const LABELS = ["רכב אחד (כל המסלול)","2 רכבים","3 רכבים","4 רכבים","5 רכבים"];
  return (
    ...
    <div style={{ minHeight:"100vh",background:"#f1f5f9",direction:"rtl",fontFamily:"'Segoe UI',system-ui,Arial,sans-serif",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",textAlign:"center",padding:28 }}>
      ...
      <div style={{ display:"flex",flexDirection:"column",gap:12,width:"100%",maxWidth:280 }}>
        <button onClick={()=>navigate(`/${numCars}cars/pick`)}
          style={{ padding:"14px 24px",borderRadius:12,border:"none",background:"#2563EB",color:"#fff",fontWeight:700,fontSize:15,cursor:"pointer" }}>
          חזור למסלולים
        </button>
        <button onClick={()=>{ saveState(`gmach_driver_${numCars}_${routeIdx}`,{}); setDelivered({}); }}
          style={{ padding:"14px 24px",borderRadius:12,border:"2px solid #d1d5db",background:"#fff",color:"#374151",fontWeight:700,fontSize:15,cursor:"pointer" }}>
          🔄 חזור לחלוקה הקודמת
        </button>
      </div>
    </div>
  ...
  return (
    <div style={{ minHeight:"100vh",background:"#f1f5f9",direction:"rtl",fontFamily:"'Segoe UI',system-ui,Arial,sans-serif",display:"flex",flexDirection:"column" }}>
      ...
      <div style={{ flex:1,display:"flex",flexDirection:"column",padding:"16px 16px 24px",maxWidth:480,margin:"0 auto",width:"100%",boxSizing:"border-box" }}>
        ...
        {/* Progress track */}
        <div style={{ marginTop:16, background:"#fff", borderRadius:14, border:"1.5px solid #e5e7eb", overflow:"hidden" }}>
          <div style={{ padding:"10px 14px 6px", fontSize:11, fontWeight:700, color:"#9ca3af" }}>
            התקדמות המסלול
          </div>

          {/* Visual dots track */}
          <div style={{ padding:"6px 14px 10px", display:"flex", alignItems:"center", gap:0, overflowX:"auto" }}>
            {stopIds.map((id, i) => {
              ...
              return (
                <div key={id} style={{ display:"flex", alignItems:"center", flexShrink:0 }}>
                  ...
                  <div style={{
                    ...
                    borderRadius:"50%",
                    background: isDone ? "#16a34a" : isCurrent ? route.color : "#e5e7eb",
                    color: isDone || isCurrent ? "#fff" : "#9ca3af",
                    display:"flex", alignItems:"center", justifyContent:"center",
                    fontSize: isCurrent ? 12 : 10, fontWeight:700,
                    border: isCurrent ? `3px solid ${route.color}` : "none",
                    boxShadow: isCurrent ? `0 0 0 3px ${route.color}33` : "none",
          ...
          {/* List */}
          <div style={{ borderTop:"1px solid #f3f4f6", maxHeight:180, overflowY:"auto" }}>
            {stopIds.map((id, i) => {
              const a = addrMap[id];
              const isDone = delivered[id];
              const isCurrent = i === currentIdx;
              return (
                <div key={id} style={{
                  ...
                  padding:"8px 14px",
                  borderBottom: i < stopIds.length-1 ? "1px solid #f9fafb" : "none",
                  background: isCurrent ? `${route.color}11` : "transparent",
                  borderRight: isCurrent ? `3px solid ${route.color}` : "3px solid transparent",
                }}>
                  <div style={{
                    width:22, height:22, borderRadius:"50%", flexShrink:0,
                    background: isDone ? "#dcfce7" : isCurrent ? route.color : "#f3f4f6",
                    color: isDone ? "#16a34a" : isCurrent ? "#fff" : "#9ca3af",
                    display:"flex", alignItems:"center", justifyContent:"center",
                    fontSize:10, fontWeight:700,
                  }}>{isDone ? "✓" : i+1}</div>
                  <span style={{
                    fontSize:13, fontWeight: isCurrent ? 700 : 400,
                    color: isDone ? "#9ca3af" : isCurrent ? "#111827" : "#374151",
                    textDecoration: isDone ? "line-through" : "none",
                    flex:1,
                  }}>{a?.name}</span>
                  {isCurrent && <span style={{ fontSize:10, color:route.color, fontWeight:700 }}>← כאן</span>}
                </div>
              );
            })}
          </div>
